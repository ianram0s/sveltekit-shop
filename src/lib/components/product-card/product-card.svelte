<script lang="ts">
    ...
    const discountPercentage = $derived(
        product.originalPrice
            ? Math.round(100 * (1 - Number(product.currentPrice) / Number(product.originalPrice)))
            : 0,
    );
</script>
...
<div
    ...
    tabindex="0"
>
    <div class="mb-4 flex aspect-square w-full items-center justify-center overflow-hidden rounded-xl">
        <img
    ...
    <div class="flex w-full flex-col items-start">
        ...
        <div class="mb-1 flex items-center gap-2">
            ...
            {#if product.originalPrice}
                <span class="text-muted-foreground text-xl line-through md:text-2xl">${product.originalPrice}</span>
                <span class="bg-destructive/10 text-destructive font-semibolde ml-1 rounded-full px-2 py-0.5 text-xs">
                    -{discountPercentage}%
                </span>
            {/if}
        </div>
