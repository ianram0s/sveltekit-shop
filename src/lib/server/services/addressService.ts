...
export class AddressService {
    ...
    static async createAddress(
        data: AddressFormInput & { userId: string },
    ): Promise<{ address?: Address; errors?: string[] }> {
        ...
        try {
            return await db.transaction(async (tx) => {
                ...
                const [insertedAddress] = await tx.insert(address).values(newAddress).returning();
            ...
            return await db.transaction(async (tx) => {
                ...
                if (data.isDefault) {
                    await tx
                        .update(address)
                        .set({ isDefault: false, updatedAt: new Date() })
                        .where(and(eq(address.userId, currentAddress.userId), eq(address.isDefault, true)));
                }
