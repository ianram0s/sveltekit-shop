...
async function loadAllSeeders(): Promise<SeederFunction[]> {
    ...
    const seederFiles = files.filter((file) => file.endsWith('.ts') && file !== 'index.ts' && !file.endsWith('.d.ts'));
    ...
    for (const file of seederFiles) {
        try {
            ...
            const exportedSeeders = Object.values(module).filter((exported): exported is SeederFunction => {
