<script lang="ts">
    ...
    function validateForm(): { isValid: boolean; errors: Record<string, string> } {
        const formData = isLogin ? ({ email, password } as SignInInput) : ({ email, password, name } as SignUpInput);
...
<div class="grid min-h-[70vh] place-items-center">
    <div class="w-full max-w-md space-y-8" in:fade={{ duration: 300, easing: quintOut }}>
        ...
        <div class="space-y-6">
            ...
            <AuthForm {isLogin} bind:email bind:password bind:name {errors} {isLoading} onSubmit={handleSubmit} />
        </div>
    </div>
</div>
