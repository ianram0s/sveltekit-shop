...
<div class="space-y-6" in:fade={{ duration: 300, easing: quintOut }}>
	<div class="flex items-center justify-between">
        <div class="flex items-center gap-3">
            <Button variant="outline" size="sm" onclick={handleBack} class="gap-2 cursor-pointer">
				...
				<div class="flex items-center justify-between p-4">
					...
					{#if data.order.order.trackingNumber}
						<div class="text-muted-foreground text-sm">
							Tracking: <span class="text-foreground font-medium"
								>{data.order.order.trackingNumber}</span
							>
						</div>
					{/if}
				</div>
