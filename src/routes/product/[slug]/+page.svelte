<script lang="ts">
    ...
    const discountPercentage = $derived(
        data.product.originalPrice
            ? Math.round(100 * (1 - Number(data.product.currentPrice) / Number(data.product.originalPrice)))
            : 0,
    );
...
<svelte:head>
    <title>{data.product.title} - Shop</title>
        ...
        <div class="flex flex-col">
            ...
            <div class="mb-5 flex items-center gap-3">
                ...
                {#if data.product.originalPrice}
                    ...
                    <span class="bg-destructive/10 text-destructive rounded-full px-3 py-1 text-sm font-semibold">
                        -{discountPercentage}%
                    </span>
                {/if}
            </div>
