<script lang="ts">
    ...
    const subtotal = $derived(() => {
        ...
        return cartState.items.reduce((sum, item) => sum + Number(item.product.currentPrice) * item.quantity, 0);
    });
...
<div class="space-y-6">
    ...
    <form method="post" action="?/validateReview" use:enhance onsubmit={() => (isProcessing = true)}>
        <!-- Order Notes -->
        <div>
            <label for="orderNotes" class="text-foreground mb-2 block text-sm font-medium">Order Notes (Optional)</label
            >
            <textarea
                id="orderNotes"
                name="orderNotes"
