...
<div class="space-y-6">
    ...
    <button
        ...
        class="border-input text-foreground bg-background hover:bg-accent hover:text-accent-foreground focus:ring-ring disabled:hover:bg-background flex w-full transform cursor-pointer items-center justify-center rounded-md border px-4 py-2.5 text-sm font-medium shadow-sm transition-all duration-200 hover:scale-[1.02] focus:ring-2 focus:ring-offset-2 focus:outline-none active:scale-[0.98] disabled:cursor-not-allowed disabled:opacity-50"
    >
        {#if isGoogleLoading}
            <div class="border-foreground/30 border-t-foreground h-5 w-5 animate-spin rounded-full border-2"></div>
        {:else}
            <GoogleIcon />
