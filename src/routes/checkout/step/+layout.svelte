    ...
    $effect(() => {
        if (browser) {
            if (cartState.items.length === 0) {
                ...
                if (
                    checkoutData &&
                    (checkoutData.customer || checkoutData.shipping || checkoutData.payment || checkoutData.review)
                ) {
        ...
        if (step <= currentStep()) {
            ...
            if (browser) {
                try {
                    ...
                    if (step === 4 && (!checkoutData.customer || !checkoutData.shipping || !checkoutData.payment)) {
                        if (!checkoutData.customer) {
                            goto('/checkout/step/customer');
                                ...
                                {#each Array.from({ length: totalSteps }, (_, i) => i + 1) as step}
                                    <div class={`flex items-center ${step < totalSteps ? 'flex-1' : ''}`}>
                                        <button
                                            ...
                                            step
                                                ? 'border-primary bg-primary text-primary-foreground shadow-sm'
                                                : 'border-border bg-background text-muted-foreground'} {currentStep() >
                                            step
                                                ? 'bg-primary border-primary'
                                                : ''}"
                                            disabled={currentStep() < step}
